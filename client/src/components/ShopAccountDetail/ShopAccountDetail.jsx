...
const ShopAccountDetail = () => {
    ...
    const renderAttribute = (attribute, value) => (
        <div className='sh-text-style'>
            <span className='sh-d'>{attribute.replace(/_/g, ' ').charAt(0).toUpperCase() + attribute.replace(/_/g, ' ').slice(1)}</span> : {value}
        </div>
    );
        ...
        <div className="shop-account-detail">
            ...
            {data.private  && (
                <div className="sh-text-style">
                    ...
                    >
                        {showPrivateDetails ? "Hide Private Details" : "View Private Details"}
                    </button>
                </div>
            )}

            <div className="sh-btn">
                <RedButton text={"BUY NOW"} type={"buyAsset"} credit={data.price} id={data._id} />
            </div>
        </div>
    );
}
